...
void loop() {
  ...
  // --------------------------------------------------------------------------------------------------------
  if(secFr == 0) {
    // ---------- 5 секунда - виводимо дату/погоду----------------------------------------------------------
    if(second == 5 && !alarm_stat) {
      sensors();
...
void getWeatherData() {
  ...
  if(ESPclient.connect(weatherHost.c_str(), 80)) {
    ESPclient.println(String("GET /v1/current.json?key="+ weatherKey +"&q=" + cityID + "&lang=" + weatherLang + " HTTP/1.1\r\n") +
                "Host: api.apixu.com\r\n" +
                "Connection: close\r\n\r\n");
  } else {
  ...
  const char* location_name = location["name"]; // "Kiev"
