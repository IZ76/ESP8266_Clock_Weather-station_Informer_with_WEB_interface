void server_init(void) {
  ...
  server.on("/kursUpdate", handle_kurs_update);
...
void handle_ConfigJSON() {
  ...
  json += "\",\"mowa\":\"";
  json += (lang==0?"UA":lang==1?"RU":lang==2?"PL":lang==3?"CZ":"DE");
  json += "\"}";
...
//======================================================================================================
void handle_kurs_update() {
  if(server.arg("kursUpdate") == "ok"){
    if(printCom) {
      printTime();
      Serial.println("Update Privat 24!");
    }
    server.send(200, "text/plain", "OK");
...
void handle_weather() {
  ...
  outKurs = server.arg("outKurs").toInt();
  ...
  if(printCom) {
    ...
    Serial.println("outForecast: " + String(outForecast) + ", outKurs: " + String(outKurs) + ", outtimeScrollStart: " + String(timeScrollStart) + ", timeScrollStop: " + String(timeScrollStop) + ", timeScrollSpeed: " + String(timeScrollSpeed));
  }
  saveConfig();
  if(outForecast) getWeatherData();
  if(outForecast) getWeatherDataz();
