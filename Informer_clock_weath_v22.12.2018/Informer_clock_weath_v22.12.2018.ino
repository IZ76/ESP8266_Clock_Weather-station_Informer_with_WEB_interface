...
String ssidAP      = "ESP-Info";                                                        // Назва точки доступу
String passwordAP  = "";                                                                // Пароль точки доступу
String weatherHost = "api.apixu.com";
String weatherKey  = "000000000000000000000000000";                                     // Ключ для отримання прогнозу погоди
...
bool fontSizeData = 0;   // 0-8 строк, 1-7 строк
bool textDown = 0;
bool dataDown = 0;
...
// ----------
  loadConfig();
  loadTimeFFS();
  blinker.attach(0.06, showAnimClock);
...
void loop() {
  ...
  if(hour == 22 && minute == 55) {                                                      // якщо наш час - то іде повідомлення для коханої
    ...
    bip();
    bip();
    printStringWithShift(("       22:55 \200\200\200 " + tMes + " \200\200\200").c_str(), timeScrollSpeed, 1);
    return;
  }
  ...
  if(secFr == 0) {
    ...
    if(second == 10 && !alarm_stat) {
      sensors();
      if(hour >= timeScrollStart && hour < timeScrollStop && !(clockNight && (hour<timeDay || hour>=timeNight))) {
        clr(1);
        if(minute % 2 == 0 || !outForecast) {
          convertDw();
          convertMonth();
    ...
    // ---------- 30 перевірка доступності WiFi мережі ---
...
void getWeatherData() {
  ...
  if(ESPclient.connect(weatherHost.c_str(), 80)) {
    ESPclient.println(String("GET /v1/current.json?key="+ weatherKey +"&q=" + urlencode(cityID) + "&lang=" + weatherLang + " HTTP/1.1\r\n") +
                "Host: api.apixu.com\r\n" +
                "Connection: close\r\n\r\n");
  } else {
  ...
  const size_t bufferSize = JSON_OBJECT_SIZE(2) + JSON_OBJECT_SIZE(3) + JSON_OBJECT_SIZE(8) + JSON_OBJECT_SIZE(21) + 600; //https://arduinojson.org/v5/assistant/
  ...
  const char* current_condition_text = current_condition["text"]; // "Суцільна хмарність"
  float current_wind_kph = current["wind_kph"]; // 6.8
  int current_wind_degree = current["wind_degree"]; // 20
  int current_pressure_mb = current["pressure_mb"]; // 1028
  int current_humidity = current["humidity"]; // 94
  int current_cloud = current["cloud"]; // 100

  String windDegString;
  if(current_wind_degree >= 345 || current_wind_degree <= 22)  windDegString = "\211";    //"Північний";
  if(current_wind_degree >= 23  && current_wind_degree <= 68)  windDegString = "\234";    //"Північно-східний";
  if(current_wind_degree >= 69  && current_wind_degree <= 114) windDegString = "\230";    //"Східний";
  if(current_wind_degree >= 115 && current_wind_degree <= 160) windDegString = "\235";    //"Південно-східний";
  if(current_wind_degree >= 161 && current_wind_degree <= 206) windDegString = "\210";    //"Південний";
  if(current_wind_degree >= 207 && current_wind_degree <= 252) windDegString = "\232";    //"Південно-західний";
  if(current_wind_degree >= 253 && current_wind_degree <= 298) windDegString = "\231";    //"Західний";
  if(current_wind_degree >= 299 && current_wind_degree <= 344) windDegString = "\233";    //"Північно-західний";
...
// =======================================================================//
//               Беремо ПРОГНОЗ!!! погоди з сайту http://api.apixu.com    // 
// =======================================================================//
void getWeatherDataz() {
  ...
  if(ESPclient.connect(weatherHost.c_str(), 80)) {
    ESPclient.println(String("GET /v1/forecast.json?key="+ weatherKey +"&q=" + urlencode(cityID) + "&lang=" + weatherLang + "&days=2" + " HTTP/1.1\r\n") +
                "Host: api.apixu.com\r\n" +
                "Connection: close\r\n\r\n");
  } else {
  ...
  const size_t bufferSize = JSON_ARRAY_SIZE(1) + JSON_OBJECT_SIZE(1) + 3*JSON_OBJECT_SIZE(3) + 2*JSON_OBJECT_SIZE(4) + JSON_OBJECT_SIZE(8) + JSON_OBJECT_SIZE(15) + JSON_OBJECT_SIZE(21) + 1530;
  DynamicJsonBuffer jsonBuffer(bufferSize);
  JsonObject& root = jsonBuffer.parseObject(line);
  if(!root.success()){
...
void sensors() {
    ...
    t3 = (sensorUl==0?0:sensorUl==1?(int)tempDs18b20:sensorUl==2?(int)celsiusSi7021:sensorUl==3?(int)tempBmp:sensorUl==4?(int)tempBme:sensorUl==5?(int)tempDht:sensorUl==6?(int)tMqtt5:0);
    t4 = (sensorUl==0?0:sensorUl==1?((int)(tempDs18b20*(tempDs18b20<0?-10:10))%10):sensorUl==2?((int)(celsiusSi7021*(celsiusSi7021<0?-10:10))%10):sensorUl==3?((int)(tempBmp*(tempBmp<0?-10:10))%10):sensorUl==4?((int)(tempBme*(tempBme<0?-10:10))% 10):sensorUl==5?((int)(tempDht*(tempDht<0?-10:10)) % 10):sensorUl==6?((int)(tMqtt5*(tMqtt5<0?-10:10))%10):0);
    t5 = (sensorUl==0?0:sensorUl==1?tempDs18b20:sensorUl==2?celsiusSi7021:sensorUl==3?tempBmp:sensorUl==4?tempBme:sensorUl==5?tempDht:sensorUl==6?tMqtt5:0);
  if(sensorHumi == 0) {
    h1 = 0;
    h2 = 0;
    h3 = 0;
